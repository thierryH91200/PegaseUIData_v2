...
//                let dataPrice = formatPrice(dataEntry[0].y)
//                str = label! + " : " + dataPrice + "\n"
...
open nonisolated class RectMarker: MarkerImage
{
    ...
    open nonisolated override func refreshContent(entry: ChartDataEntry, highlight: Highlight)
    {
        ...
        for  dataSets in chartView!.data!.dataSets
        {
            ...
            if !dataEntry.isEmpty
            {
                var data = dataSets.valueFormatter.stringForValue(dataEntry[0].y, entry: dataEntry[0], dataSetIndex: 0, viewPortHandler: nil)
                let dataPrice =                   formatPrice(dataEntry[0].y)
