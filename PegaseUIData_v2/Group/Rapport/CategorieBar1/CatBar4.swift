...
struct DGBarChart7Representable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, ChartViewDelegate {
        ...
        func chartValueSelected(_ chartView: ChartViewBase,
            ...
            if parent.viewModel.dataGraph.indices.contains(index) {
                let item = parent.viewModel.dataGraph[index]
                parent.onSelectBar?(index, item)
            }
        }

        func chartValueNothingSelected(_ chartView: ChartViewBase) {
            // Notify SwiftUI to clear the selection (pass nil via onSelectBar with index -1)
            parent.onSelectBar?(-1, DataGraph(name: "", value: 0, color: .clear))
        }
    }
    ...
    func updateNSView(_ nsView: BarChartView, context: Context) {
        setData(on: nsView, with: viewModel.dataGraph)
    }
