...
struct DGBarChart4Representable: NSViewRepresentable {
    ...
    var onSelectBar: (([EntityTransaction]) -> Void)? = nil
...
//            guard index >= 0, index < parent.labels.count else { return }
            ...
            guard let globalMin = all.first?.datePointage else { return }
            ...
            if self.fullFilteredCache.isEmpty {
                self.fullFilteredCache = all.filter { tx in
                    tx.datePointage >= rangeStart && tx.datePointage < rangeEndExclusive
                }
            }
            ...
            let monthFiltered = self.fullFilteredCache.filter { tx in
                tx.datePointage >= monthStart && tx.datePointage < monthEndExclusive
            }
            ...
            // Notify SwiftUI parent directly with filtered transactions
            DispatchQueue.main.async {
                self.parent.onSelectBar?(filtered)
            }
        }
        ...
        public func chartValueNothingSelected(_ chartView: ChartViewBase) {
            ...
            DispatchQueue.main.async {
                ...
                self.parent.onSelectBar?(restored)
            }
        }
    }
